phina.namespace(function() {
  ...
  phina.define("MotionController", {
    ...
    init: function() {
      ...
      this.on("attached", e => {
        this.transitionTweener = Tweener().attachTo(this.target);
    ...
    update: function(app) {
      ...
      target.$t.update(app.deltaTime / 1000);
    },
  ...
  phina.define("Motion", {
    ...
    update: function(controller) {
      if (this._action) {
        this._timeEventListeners.forEach(t => {
          if (!t.fired && this._action.time <= t.time) {
            t.listener();
            t.fired = true;
          }
        });
